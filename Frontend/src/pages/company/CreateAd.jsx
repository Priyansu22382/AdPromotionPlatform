...
import { axiosInstance } from "../../lib/axios";
import { useNavigate } from "react-router-dom";
import { PlusCircle, Image, Text, Calendar, Car, Tag, DollarSign, UploadCloud, ShieldCheck } from "lucide-react";
    ...
    <div className="min-h-screen w-screen flex flex-col items-center p-6 bg-gradient-to-br from-gray-900 to-black text-gray-200 font-sans">
      ...
      <div className="z-10 w-full max-w-2xl p-8 rounded-3xl border border-gray-700 bg-gray-800/80 shadow-2xl shadow-gray-950/50 backdrop-blur-md transition-shadow duration-300 hover:shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block mb-2 text-gray-400">
              <UploadCloud className="inline-block mr-2 text-teal-400" size={20} />
              Upload Ad Image
            </label>
            <div className="relative border-2 border-dashed border-gray-700 rounded-xl p-6 text-center transition-all duration-300 hover:border-teal-500 hover:bg-gray-800">
              <input
                type="file"
                accept="image/*"
                onChange={handleImageChange}
